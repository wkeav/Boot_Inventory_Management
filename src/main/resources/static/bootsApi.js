...
const handle500Error = (json) => {
  ...
  const fetchAllBoots = (cb) => {
    fetch("/api/v1/boots/")
      ...
      .then(handle500Error)
      .then(json => {
        console.log("Boots data:", json); // This logs the boot data
        cb(json); // This sends the data to the callback for rendering
      })
      .catch(renderError);
  }
  ...
  fetchAllBoots(
    renderBootsListCallback(
      bootsTableBody = document.getElementById("bootsTableBody")
    )
  );
